...
`thread.settle` settles the active conversation with the current bot, or restores it when it is
already settled. Its default shortcut is `mod+shift+s`, and it does not run while the terminal has
focus.
...
The command palette searches active conversation titles, projects, branches, user messages, and
final bot responses across connected environments. Message matches show one labeled excerpt while
keeping the conversation's project, branch, and machine context visible. Message search begins after
two characters and uses SQLite's ASCII case-insensitive matching.

The full command list and the current defaults are shown in **Settings > Keybindings**, which
always matches the build you are running. Use that rather than a copied list.

`chat.new` and `chat.newLocal` both start a conversation with the current bot through the same path.
The new conversation inherits the project you were in, along with model and mode selections. Branch,
worktree, and environment mode always come from your configured defaults, not from the conversation
you were looking at. To keep a worktree, use the explicit action in the branch toolbar that starts a
conversation in this worktree. The only difference between the two commands is that `chat.new` opens
a project chooser first when the current sidebar has more than one project.

Background submission from a new conversation is the exception. `mod+enter` starts that conversation
and opens another conversation with the current bot, using the same workspace mode and base branch.
**New worktree** remains selected, but the new conversation does not reuse the worktree created for
