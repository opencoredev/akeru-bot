...
`akeru pair` finds the running server (the shared `~/.t3` install, or the current worktree's dev server when run inside one), issues a one-time pairing token, and prints the pairing URL as a QR code you can scan from your phone.
...
If no server is running, `akeru pair` says so and points you at `npx akeru-bot serve` or `npx akeru-bot connect`.
...
backend with the same server-side behavior as `akeru serve --tailscale-serve`, then the server asks
...
Run the server with `akeru serve`.

```bash
npx akeru-bot serve --host "$(tailscale ip -4)"
```

`akeru serve` starts the server without opening a browser and prints:
...
Use `akeru serve --help` for the full flag reference. It supports the same general startup options as the normal server command, including an optional `cwd` argument.
...
```bash
npx akeru-bot serve --tailscale-serve --tailscale-serve-port 8443
```
...
1. `akeru serve` issues a one-time owner pairing token.
...
Use `akeru auth` to manage access after the initial pairing flow.
...
Use `akeru auth --help` and the nested subcommand help pages for the full reference.
...
- Use `akeru auth` to revoke credentials or sessions you no longer trust.
