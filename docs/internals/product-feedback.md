...
A network or runtime failure after GitHub issue creation begins is marked `unknown` because retrying an ambiguous create could duplicate the issue. Unknown rows are quarantined from automatic delivery, but the same 90-day retention limit still deletes them. The transition and every scheduled run with outstanding unexpired unknown rows emit structured `feedback.github_delivery_unknown` or `feedback.github_delivery_unknown_outstanding` errors to Worker logs.

To reconcile an unknown row before its expiry, query `feedback_id`, `received_at`, `expires_at`, `github_last_error_code`, and `payload_json` from `akeru_feedback_inbox WHERE github_issue_status = 'unknown'` with authenticated Wrangler D1 tooling. Search all repository issues for the feedback ID. If an issue exists, set the row to `delivered` with its issue number and URL; if none exists, set it to `failed` with `github_next_attempt_at` set to the current ISO timestamp so the next drain retries. Never retry before checking GitHub.
