...
describe("hostedPairing", () => {
  ...
  it("reads hosted pairing host and query token parameters", () => {
    const url = new URL("https://example.test/pair?host=100.64.1.2:3773&token=ABCD1234");
  ...
  it("ignores incomplete hosted pairing requests", () => {
    expect(
      hasHostedPairingRequest(new URL("https://example.test/pair?host=backend.example.com")),
    ).toBe(false);
    expect(hasHostedPairingRequest(new URL("https://example.test/pair?token=ABCD1234"))).toBe(
      false,
    );
  });

  it("does not treat any origin as a hosted static app when VITE_HOSTED_APP_URL is unset", () => {
    vi.stubEnv("VITE_HOSTED_APP_URL", "");
    vi.stubEnv("VITE_HTTP_URL", "");
    vi.stubEnv("VITE_WS_URL", "");

    expect(isHostedStaticApp(new URL("https://example.test/"))).toBe(false);
    expect(isHostedStaticApp(new URL("https://example.test/pair"))).toBe(false);
  });
