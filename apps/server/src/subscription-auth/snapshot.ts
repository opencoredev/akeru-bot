...
const SUBSCRIPTION_INSTANCE: Readonly<Record<string, SubscriptionProviderId | undefined>> = {
...
export function subscriptionDependentBots(
  ...
  return bots.flatMap((bot) => {
    ...
    const provider = providers.find((candidate) => candidate.instanceId === bot.engine?.provider);
    if (provider?.auth.type === "apiKey") return [];
    const subscriptionProvider = SUBSCRIPTION_INSTANCE[bot.engine.provider];
...
export function buildProviderAccessCapabilities(
  ...
  const subscriptionRows = SUBSCRIPTION_ACCESS.map((entry) => {
    ...
    return {
      ...
      ...(status?.reconnectAction ? { reconnectAction: status.reconnectAction } : {}),
  ...
  return [
    ...subscriptionRows,
    {
      ...
      dependentBots: subscriptionById.get("xai")?.dependentBots ?? [],
    },
    {
      id: "supergrok",
      label: "SuperGrok",
    ...
    ...row,
    dependentBots: "dependentBots" in row ? row.dependentBots : [],
    dependentRoutines: [],
  }));
}
