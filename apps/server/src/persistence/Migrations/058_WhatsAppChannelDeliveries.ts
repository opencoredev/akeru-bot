...
export default Effect.gen(function* () {
  ...
  yield* sql`ALTER TABLE channel_deliveries RENAME TO channel_deliveries_before_whatsapp`;
  yield* sql`
  ...
  yield* sql`
    ...
    SELECT
      message_id, bot_id, thread_id, provider, external_thread_id, status, requested_at, sent_at
    FROM channel_deliveries_before_whatsapp
  `;
  yield* sql`DROP TABLE channel_deliveries_before_whatsapp`;
  yield* sql`
