      ...
      assert.deepEqual(snapshot.threads, [
        {
          ...
          latestTurn: {
            ...
            requestMessageId: asMessageId("message-request-1"),
            respondingBotId: null,
            sourceProposedPlan: {
  ...
  it.effect("uses projection_threads.latest_turn_id for targeted thread latest turn queries", () =>
    Effect.gen(function* () {
      ...
      if (threadDetail._tag === "Some") {
        ...
        assert.equal(
          threadDetail.value.latestTurn?.requestMessageId,
    ...
    Effect.gen(function* () {
      ...
      yield* sql`
        ...
        VALUES
          ...
          (${ORCHESTRATION_PROJECTOR_NAMES.mcpServers}, 3, '2026-04-03T00:00:40.000Z'),
      ...
      assert.equal(
        commandReadModel.threads[0]?.latestTurn?.requestMessageId,
        asMessageId("message-user-2"),
      );
      ...
      assert.equal(
        shellSnapshot.threads[0]?.latestTurn?.requestMessageId,
        asMessageId("message-user-2"),
      );
      ...
      assert.equal(
        fullSnapshot.threads[0]?.latestTurn?.requestMessageId,
        asMessageId("message-user-2"),
      );
    }),
  );
